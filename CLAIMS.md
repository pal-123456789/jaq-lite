...
| 14 | The suite is green | `cargo test` | 190 `#[test]` functions across every target, all passing, 0 failing; the figure is grepped out of `src/` and `tests/` by `the_ledger_counts_the_tests_the_tree_actually_has` rather than typed here, because it had already gone stale twice |
