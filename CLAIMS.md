...
| 14 | The suite is green | `cargo test` | 177 passing, 0 failing, summed across every target |
...
| 25 | The platforms table names both targets this was built on, and the compiler version in it is the one `Cargo.toml` pins | `cargo test --test claims -- --nocapture --test-threads=1` | 10 passing; two hosts, both rustc 1.98.0 (measured 2026-08-30) |
